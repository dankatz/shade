...
near <- stack("C:/Users/dsk856/Desktop/TX_remote_sensing/nearmap/EPSG32614_Date20200509_Lat30.285278_Lon-97.739535_Mpp0.597/EPSG32614_Date20200509_Lat30.285278_Lon-97.739535_Mpp0.597.jpg")
plotRGB(near)
crs(near) <-CRS('+init=EPSG:32614')
extent_near <- extent(near)

#near2 <- projectRaster(from = near, to = chm_small_rast_s)
#plotRGB(near2)
#?projectRaster
...
las <- readLAS("C:/Users/dsk856/Desktop/TX_remote_sensing/stratmap17-50cm_3097433a3.laz")
las
#plot(las)
...
#sun position
#based on: http://matthewkling.github.io/media/rayshader/
sp <- seq(ISOdate(2020,7,22,0), ISOdate(2020,7,22,24), by="min") %>% 
  ...
  filter(zenith >= -10, # exclue nighttime
         id %% 20 == 0) # keep every third frame

#calculate shade
azimuth = sp$azimuth[10]
zenith = sp$zenith[10]
...
#near_rgb_array %>%
  ...
  add_shadow(ray_shade(nSDM_matrix, sunaltitude=20, #anglebreaks=seq(zenith-4, zenith+4, 1),
                       sunangle = azimuth, zscale=0.07),
             max_darken=0.1) %>% #, multicore = TRUE
  plot_map()

?ray_shade
#2D version
ambmat <- ambient_shade(nSDM_matrix, zscale = 0.59)

nSDM_matrix %>% 
    ray_shade(nSDM_matrix) %>% 
  #sphere_shade(nSDM_matrix, colorintensity=0.5) %>%
  add_shadow(ray_shade(nSDM_matrix, sunaltitude=20, #anglebreaks=seq(zenith-4, zenith+4, 1),
                       sunangle = azimuth, zscale=0.07),
             max_darken=0.1) %>% 
  plot_map(near_rgb_array)
...
#calculate shade at a particular time and date
azimuth = sp$azimuth[50]
zenith = sp$zenith[50]
raymat <- ray_shade(nSDM_matrix, zscale = 0.59, #lambert = TRUE, sunaltitude=20, 
                    anglebreaks=seq(zenith-4, zenith+4, 1), sunangle = azimuth) #?ray_shade
nSDM_matrix %>%
...
# save animation
dev.off()
img %>% image_animate(fps = 20) %>% image_write("animation.gif")
